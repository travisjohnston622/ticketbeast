...
//        $response->assertViewHas($response->data('concert')->is($concert)); TODO: Why doesn't this work?
    }
    ...
    function promoters_can_edit_their_own_unpublished_concerts()
    {
        ...
        tap($concert->fresh(), function ($concert) {
            ...
            $this->assertEquals(6527, $concert->ticket_price);
            $this->assertEquals(10, $concert->ticket_quantity);
        });
    }

    /** @test */
    function promoters_can_not_edit_other_unpublished_concerts()
    {
        $user = factory(User::class)->create();
        $otherUser = factory(User::class)->create();
        $concert = factory(Concert::class)->create($this->oldAttributes([
    ...
    /** @test */
    function promoters_can_not_edit_published_concerts()
    {
        $user = factory(User::class)->create();
    ...
    /** @test */
    function guest_can_not_edit_concerts()
    {
        $user = factory(User::class)->create();
        $concert = factory(Concert::class)->create($this->oldAttributes([
            'user_id' => $user->id,
